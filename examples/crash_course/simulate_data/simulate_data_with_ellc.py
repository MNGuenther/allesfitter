...
params['b_a'] = 0.019771142 * params['b_K'] * params['b_period']
pprint(params)
...
np.savetxt('Leonardo.csv', X, delimiter=',', header=header)
...
np.savetxt('Michelangelo.csv', X, delimiter=',', header=header)
...
rv_err_Donatello = 2e-3*np.ones_like(rv_Donatello)
header = 'time,flux,flux_err'
X = np.column_stack(( time_Donatello, rv_Donatello, rv_err_Donatello ))
np.savetxt('Donatello.csv', X, delimiter=',', header=header)
...
np.savetxt('Raphael.csv', X, delimiter=',', header=header)
...
plt.tight_layout()
fig.savefig('all_data.png', bbox_inches='tight')