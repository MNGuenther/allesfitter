...
def mcmc_output(datadir):
    ...
    #::: security check
    if os.path.exists(os.path.join(config.BASEMENT.outdir,'mcmc_fit.jpg')):
    ...
    fig.savefig( os.path.join(config.BASEMENT.outdir,'mcmc_chains.jpg'), dpi=100, bbox_inches='tight' )
    plt.close(fig)

    #::: plot the corner
    fig = plot_MCMC_corner(reader)
    fig.savefig( os.path.join(config.BASEMENT.outdir,'mcmc_corner.jpg'), dpi=80, bbox_inches='tight' )
    plt.close(fig)
